...
export function listFiles(db: Sqlite3.DB, options: ListOptions): DbReadableFileV2[] {
  const clauses = [
    ...(options.paths?.length || options.ignore?.length
      ...
      : []),
    `SELECT meta,path,content,isDeleted,status,updatedAt FROM FileV2`,
    ...(options.filters?.length || options.paths?.length || options.ignore?.length
      ? [
          ...
          [
            ...
            ...(options.paths?.length
              ? [`EXISTS ( SELECT 1 FROM Include WHERE FileV2.path GLOB Include.pattern)`]
              : []),
            ...(options.ignore?.length
              ? [`NOT EXISTS ( SELECT 1 FROM Ignore WHERE FileV2.path GLOB Ignore.pattern)`]
              : []),
          ].join(" AND "),
        ]
      : []),
...
export function searchFiles(db: Sqlite3.DB, options: SearchOptions): DbReadableFileV2[] {
  const clauses = [
    ...(options.paths?.length || options.ignore?.length
      ...
      : []),
    `SELECT FileV2.meta,FileV2.path,FileV2.content,isDeleted,status,updatedAt FROM FileV2 JOIN FileFts ON FileV2.path = FileFts.path`,
    ...[
      "WHERE",
      [
        `FileFts MATCH :query`,
        ...(options.filters?.map(([col, op]) => `FileV2.${col} ${op} :${col}`) ?? []),
        ...(options.paths?.length ? [`EXISTS ( SELECT 1 FROM Include WHERE FileV2.path GLOB Include.pattern)`] : []),
        ...(options.ignore?.length ? [`NOT EXISTS ( SELECT 1 FROM Ignore WHERE FileV2.path GLOB Ignore.pattern)`] : []),
      ].join(" AND "),
    ],
